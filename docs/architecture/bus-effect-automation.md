...
- 一个持续声源在入水后持续变闷
- 优先选择持续 `music` 或持续环境声，而不是只依赖离散 `one-shot`
...
- 仅使用脚步这类离散 `one-shot` 作为主要验证器，容易把“上一脚的尾音”和“当前环境状态”混在一起
- 这会让验证结果看起来像是 effect 状态反相，实际上未必是 backend 行为错误
- 因此 `[4]` 的主观听感验证应优先依赖持续内容
- `one-shot` 更适合作为补充观察项，而不是主要验收依据
...
- 做复杂 snapshot 叠层系统
- 为了过渡而新增最小 fade 特判
- 用 `one-shot` 示例单独承担全部低通验证责任

补充边界：

- 未来仍然可能存在 `one-shot` 事件经过 low-pass 或其他 bus effect 的实际需求
- 这一类需求当前并未被否定，也不应从系统设计上排除
- 只是现阶段先不把它作为 `[4]` 的主验证路径
- 待持续声源版本的 `Underwater` example 跑通后，再回头单独验证 `one-shot -> bus effect` 的听感与时序语义
